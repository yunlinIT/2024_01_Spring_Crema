...
public class WebCrawler13 { 
    ...
    public static List<Cafe> crawlCafes() { // 카페 크롤링 메서드 시작
        ...
        String inputKey = "대전 동구 카페"; // 검색어 문자열 변수 선언
        ...
        List<WebElement> elements = driver.findElements(By.className("TYaxT")); // 검색 결과 WebElement 목록 가져오기 // 사이트에서 전체 매장을 찾은 뒤 코드를 읽는다
        ...
        for (WebElement e : elements) { // WebElement 목록을 순회하며
            ...
            try { // 예외 처리 시작
                ...
                button.click(); // 버튼 클릭
                List<WebElement> dayElements = driver.findElements(By.xpath("//span[@class='i8cJw']")); // 요일 WebElement 목록 가져오기
                List<WebElement> timeElements = driver.findElements(By.xpath("//div[@class='H3ua4']")); // 시간 WebElement 목록 가져오기
            ...
            try { // 예외 처리 시작
                ...
                for (int i = 0; i < menuEles.size(); i++) { // WebElement 목록을 순회하며
                    String temp = menuEles.get(i).getText() + ":" + priceEles.get(i).getText() + ";"; // 메뉴와 가격을 ':'로 연결하여 temp 문자열에 저장
            ...
            List<WebElement> imageElements = driver.findElements(By.cssSelector("div.K0PDV._div")); // 이미지 WebElement 목록 가져오기
    ...
    private static String extractImageUrlFromStyleAttribute(String styleAttribute) { // 스타일 속성에서 이미지 URL을 추출하는 메서드 시작
        String imageUrl = ""; // 이미지 URL 문자열 변수 선언
        if (styleAttribute != null && styleAttribute.contains("background-image: url(")) { // 스타일 속성이 null이 아니고 'background-image: url('을 포함하는 경우
            int startIndex = styleAttribute.indexOf("url(") + 4; // 시작 인덱스 설정
            int endIndex = styleAttribute.indexOf(")", startIndex); // 끝 인덱스 설정
            imageUrl = styleAttribute.substring(startIndex, endIndex).replaceAll("'", "").replaceAll("\"", ""); // 이미지 URL 추출하여 imageUrl 변수에 저장
        }
