...
$(document).ready(function() {
	...
	// 페이지 로드 시 전체 카페 목록을 가져오는 Ajax 요청
/*     $.ajax({
        url: "/usr/findcafe/searchList", // 실제 Ajax 요청을 처리할 URL
        type: "GET", // GET 요청으로 전체 카페 목록을 가져옴
        contentType: "application/json", // JSON 형식으로 데이터를 전송하지 않으므로 생략 가능
        success: function(cafeList) {
            // Ajax 요청 성공 시 처리
            console.log("Cafe list:", cafeList); // 받은 응답을 콘솔에 출력
            alert("카페 리스트 : " + cafeList.length);
            // 받은 응답을 사용하여 카페 목록을 업데이트하거나 화면에 출력하는 등의 작업 수행
            // 예시: 카페 목록을 업데이트하는 함수 호출
    ...
    function loadFilteredCafes(keyword, page){
        ...
        function updateCafeList(cafeList) {
            ...
            var searchResult = $("#search-result");		// 2024-03-24 daniel
            ...
            cafeList.forEach( function(cafe) {
                ...
                searchResult.append(cafeItem);		// 2024-03-24 daniel
